        ...
        for (var x in contractAbi) {
            if (counter == 0) {
                for (var y = 0; y < contractAbi[x].abi.length; y++) {
                    contractAbiArray.push(<ContractsFunction abi={contractAbi[x].abi[y]} />);
                }
            }
        }
                            ...
                            return (
                                <div id={contractName} className="contract-container" key={index}>
                                    ...
                                    <form className="gas-estimate-form">
                                        <button className="input text-subtle">Contract address</button>
                                        <input id="SimpleStorageContract_gas" type="number" className="inputs" />
                                    </form>
                                    {
                                        compiled ? this.renderContractFunction(compiled.contracts[fileName]) : ''
                                    }
                                    {
...
class ContractsFunction extends React.Component {
    constructor(props) {
        super(props);
    }
    render() {
