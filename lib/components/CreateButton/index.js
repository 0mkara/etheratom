...
import { setInstance, setDeployed, addNewLogs } from '../../actions'
        ...
        try {
            ...
            contractInstance.on('instance', instance => {
                instance.events.allEvents({ fromBlock: 'latest' })
                    .on('logs', (logs) => {
                        this.props.addNewLogs({ payload: logs });
                    })
                    .on('data', (data) => {
                        this.props.addNewLogs({ payload: data });
                    })
                    .on('changed', (changed) => {
                        this.props.addNewLogs({ payload: changed });
                    })
                    .on('error', (error) => {
...
export default connect(mapStateToProps, { setDeployed, setInstance, addNewLogs })(CreateButton);
