	...
	async createTabView() {
		...
		class CompiledContract extends React.Component {
			...
			async componentDidMount() {
				...
				const gas = await that.helpers.getGasEstimate(that.coinbase, bytecode);
		...
		class Contracts extends React.Component {
			...
			render() {
				const { contractName, compiled } = this.props;
		...
		ReactDOM.render(<TabView compiled={that.compiled}/>, document.getElementById('tab_view'));
	}
	async viewCompiled(compiled) {
		...
		for (contractName in compiled.contracts) {
			try {
				const bytecode = compiled.contracts[contractName].bytecode;
				const ContractABI = JSON.parse(compiled.contracts[contractName]["interface"]);
				const estimatedGas = await self.helpers.getGasEstimate(self.coinbase, bytecode);
				inputs = [];
		...
		class executeButton extends React.Component {
			...
			async _handleSubmit() {
				try {
					...
					const contractInstance = await self.helpers.deploy(contract, params);
					contractInstance.on('address', address => {
						contract.options.address = address;
						self.setExecutionView(contractName, abiDef, bytecode, constructorParams, params, contract);
					});
