...
export default class View {
	...
	setExecutionView(contractName, abiDef, bytecode, constructorParams, params, contract) {
		...
		class functionABI extends React.Component {
			...
			componentDidMount() {
				return self.helpers.constructFunctions(abiDef, (error, childFunctions) => {
					if(!error) {
						this.setState({ childFunctions: this.state.childFunctions.concat([childFunctions]) });
					}
				});
			}
			_handleChange(event) {
			...
			render() {
				return React.createElement('div', {
					...
					return React.createElement('form', {
						onSubmit: this._handleSubmit.bind(childFunction[0]),
						key: i,
						ref: childFunction[0]
					}, React.createElement('input', {
						type: 'submit',
