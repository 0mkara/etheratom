...
const deploy = async(contract, params, contractName, web3) => {
    try {
        params = params.map(param => {
            var reg = /\w+(?=\[\d+\])/g;
            return (param.type.match(reg) || param.type.endsWith('[]')) ? param.value.search(', ') > 0 ? param.value.split(', ') : param.value.split(',') : param.value;
        });
...
let call = async(args, web3) => {
    ...
    try {
        ...
        params = params.map(param => {
            var reg = /\w+(?=\[\d+\])/g;
            if (param.type.match(reg) || param.type.endsWith('[]')) {
