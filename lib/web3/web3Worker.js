    ...
    try {
        ...
        contract.contract.deploy({
            ...
            .then(instance => {
                ...
                globalContract.options.address = instance.options.address;
...
let call = async (args, web3) => {
    ...
    web3.eth.defaultAccount = coinbase;
    process.send({ callResult: args });
    try {
        // Prepare params for call
        params = params.map(param => {
            if (param.type.endsWith('[]')) {
                const callResult1 = param.value.search(', ') > 0 ? param.value.split(', ') : param.value.split(',');
                process.send({ callResult1 });
                return false;
            }
            if (param.type.indexOf('int') > -1) {
                const utilsObject = new web3.utils.BN(param.value);
                process.send({ callResult: utilsObject });
                return false;
            }
            process.send({ callResult: param.value });
            return false;
        });
        ...
        if (abiItem.type === 'fallback') {
            ...
            process.send({ callResult: result });
            return false;
        }
        ...
        if (abiItem.constant === false || abiItem.payable === true) {
            ...
            const result = await globalContract.methods[abiItem.name]().send({ from: coinbase, value: abiItem.payableValue });
            process.send({ callResult: result });
            return false;
        }
        if (params.length > 0) {
        ...
        const result = await globalContract.methods[abiItem.name]().call({ from: coinbase });
        process.send({ callResult: result });
    }
    catch (e) {
        process.send({ error: e.message });
