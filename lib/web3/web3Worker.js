...
let create = async (args, web3) => {
    ...
    try {
        ...
        try {
            ...
            let contractObject = JSON.parse(JSON.stringify({ contract, contractName: args.contractName }));
            process.send('This is Demo Message 46');
...
const deploy = async (contract, params, web3) => {
    process.send({ mesage: 'This is demo data message' });
...
process.on('message', async (message) => {
    try {
        ...
        if (message.action === 'get_balances' && message.hasOwnProperty('coinbase')) {
            await getBalances(message.coinbase, web3);
        }
