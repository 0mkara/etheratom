...
const deploy = async(contract, params, contractName, web3) => {
    try {
        params = params.map(param => {
            return (param.type.match(/[w\[\d\]]/g) || param.type.endsWith('[]')) ? param.value.search(', ') > 0 ? param.value.split(', ') : param.value.split(',') : param.value;
        });
...
let call = async(args, web3) => {
    ...
    try {
        ...
        params = params.map(param => {
            if (param.type.match(/[w\[\d\]]/g) || param.type.endsWith('[]')) {
