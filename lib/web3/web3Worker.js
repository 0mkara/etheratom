...
let checkConnection = (web3) => {
    try {
        let haveConn = web3.currentProvider;
        if (!haveConn) {
...
let create = async (args, web3) => {
    ...
    const { coinbase, password, abi, bytecode, gas, contractName } = args;
    if (!coinbase) {
        const error = new Error('No coinbase selected!');
        process.send({ error: error.message });
    }
    ...
    try {
        ...
        try {
            ...
            // process.send({ jsonInterface: contract.options.jsonInterface, contractName })
            process.send({ options: contract.options, contractName })
            return contract;
        } catch (e) {
            throw e;
        }
    ...
    try {
        ...
        contract.deploy({ arguments: params })
          ...
          .then(instance => {
              ...
              process.send({ address: instance.options.address, contractName });
          });
    } catch (e) {
        console.log(e);
        process.send({ error: e.message });
    ...
    try {
        ...
        params = params.map(param => {
            ...
            if (param.type.indexOf('int') > -1) {
                return new web3.utils.BN(param.value);
            }
            return param.value;
        });
...
process.on('message', async (message) => {
    try {
        ...
        if (message.action === 'create' && message.hasOwnProperty('argumentsForCreate')) {
            ...
            const { params, contractName } = args
