...
import { SET_COMPILED, ADD_INTERFACE, SET_COMPILING } from '../actions/types'
	...
	async compile(editor) {
		...
		if(filePath.split('.').pop() == 'sol') {
			...
			try {
				...
				for(const [contractName, contract] of Object.entries(compiled.contracts)) {
					const ContractABI = JSON.parse(contract.interface);
					// Add interface to redux
					this.store.dispatch({ type: ADD_INTERFACE, payload: { contractName, interface: ContractABI } });
				}
