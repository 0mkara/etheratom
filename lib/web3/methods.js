...
export default class Web3Helpers {
    constructor(store) {
        ...
        this.web3Connection.send({ action: 'set_rpc_ws', rpcAddress, websocketAddress });
    }
    ...
    killWeb3Connection() {
        if (this.web3Connection !== 'undefined') {
            this.web3Connection.kill();
        } else {
            return;
        }
    }
    ...
    async getGasEstimate(coinbase, bytecode) {
        ...
        web3Connection.send({ action: 'set_rpc_ws', rpcAddress, websocketAddress });
        ...
        web3Connection.on('message', (message) => {
            if (message.hasOwnProperty('connected') && message['connected']) {
                ...
                message.send({ action: 'set_rpc_ws', coinbase, bytecode });
            }
        });
    ...
    async getBalance(coinbase) {
        ...
        this.web3Connection.send({ action: 'set_rpc_ws', action_2: 'get_balances', coinbase });
        // this.web3Connection.on('message', (message) => {            
