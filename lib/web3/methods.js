    ...
    async compileWeb3(sources) {
        ...
        try {
            ...
            if (foundAt !== undefined) {
                ...
                console.log(output);
                // solcWorker.send({ command: 'compile', payload: input, version: requiredSolcVersion, snapshot: s });
            } else {
                ...
                if(m.compiled) {
                    ...
                    if(m.solc) {
                        ...
                        if(cachedSolc.length === 0) {
                            this.store.dispatch({ type: CACHED_SOLC, payload: [m.solc] });
                        } else {
                            cachedSolc.push(m.solc);
                            this.store.dispatch({ type: CACHED_SOLC, payload: cachedSolc });
                        }
                    }
                    solcWorker.kill();
                }
