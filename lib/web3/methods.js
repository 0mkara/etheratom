	...
	async call(coinbase, password, contract, methodItem, params) {
		...
		try {
			...
			if(methodItem.constant === false || methodItem.payable === true) {
				...
				if(params.length > 0) {
					const result = await contract.methods[methodItem.name](...params).send({ from: coinbase, value: methodItem.payableValue });
					return result;
				}
				const result = await contract.methods[methodItem.name]().send({ from: coinbase, value: methodItem.payableValue });
				return result;
			}
			if(params.length > 0) {
