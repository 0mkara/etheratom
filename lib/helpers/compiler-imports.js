...
export async function handleLocalImport(path, filename, fileRoot) {
    ...
    const content = fs.readFileSync(fileRoot + filename, o);
    const response = { content, fileRoot };
    return response;
}
async function getHandlers() {
    return [
        { type: 'local', match: /^(?!https?:\/\/)?(?!www.)?(?!github.com)?(\.\.|\.)*?(\/*[\w+/]*\/)*?(\w+.sol)/g, handle: async (match, fileRoot) => { return await handleLocalImport(match[2], match[3], fileRoot) } },
...
export async function resolveImports(fileRoot, sourcePath) {
