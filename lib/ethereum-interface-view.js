...
module.exports = AtomSolidityView = (function() {
	...
	AtomSolidityView.prototype.createCompilerOptionView = function() {
		compilers = [{
			compiler: 'solcjs',
			desc: 'Javascript VM'
		}, {
		...
		atom.config.observe('etheratom.executionEnv', function(newValue) {
			let executionEnv;
			executionEnv = newValue;
			createCompilerEnvList = React.createClass({
				displayName: 'envList',
				getInitialState: function() {
					return {
						compilerValue: compilers[0].compiler === executionEnv ? compilers[0].compiler : compilers[1].compiler
					};
				},
